...
滚动/选词说明：tmux 回滚（scroll_offset）与 pickup 主屏的按键滚动完全保留；
...
class EmbedPane(Widget):
    ...
    def _capture_loop(self) -> None:
        ...
        while not self._stop.is_set():
            try:
                ...
                pane_w, pane_h = self._pane_size()
                text = embed.capture(name, scroll_offset, pane_h)
                ...
                if text is None:
                    ...
                    if now - last_state_at >= STATE_POLL_INTERVAL:
                        ...
                        if state is not None:
                            last_state = state
                    state = last_state
                    frame_key = (generation, scroll_offset, pane_w, pane_h, text)
    ...
    def render(self) -> "Text":
        ...
        if self._grid is None:
            # 连接/抓帧是后台实现细节，不能暴露成用户可见的中间态。已有会话先
            # 展示扫描层已经具备的详情；刚启动、尚无详情的新会话直接展示空白终端
            # 画布，首帧到达后无缝替换。任何情况下都不渲染“连接中”之类占位页。
            if self._detail_renderer is not None:
                rendered = self._detail_renderer()
                return rendered if isinstance(rendered, Text) else Text(str(rendered))
            return Text()
        out = Text()
        rows = embed.grid_to_text(self._grid)
        for i, (line, spans) in enumerate(rows):
            row_text = Text(line)
            for start, end, style in spans:
                row_text.stylize(style, start, end)
            out.append(row_text)
            if i != len(rows) - 1:
                out.append("\n")
        return out
    ...
    async def _on_key(self, event: events.Key) -> None:
        ...
        event.stop()
        event.prevent_default()
        if self.scroll_offset > 0 and event.key in ("up", "down"):
    ...
    def _scroll(self, delta: int) -> None:
        ...
        if not name or self.dead:
            return
        new_offset = max(0, min(self.scroll_offset + delta, self._history_size))
