...
`pickup` 是指向项目 `sc.py` 的符号链接。代码更新后若界面未变化，先核对 `command -v pickup`、`readlink "$(command -v pickup)"` 和源码哈希，确认没有运行另一台机器或旧目录中的副本。
