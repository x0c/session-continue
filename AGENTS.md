...
- `docs/MAINTAINER_GUIDE.md`：修改、评审、排查或优化标题生成、扫描排序、扫描性能/启动耗时、界面列宽、配色/可读性（含浅色背景终端适配）、状态列/会话存活判断、会话预览、运行时边界（含跨运行时接手提示词的对话摘录）、`agent_api.py` 机器接口（含新增字段/参数、只读边界取舍、`AgentApiTests` 测试写法）、打包发布和 GitHub 开源维护前必读。
