...
| `Space` | 全屏按时间顺序预览“用户消息 → 最终答复”聊天记录；默认定位最新一轮，`Home` / `End` 跳到首尾，`Space` / `q` 关闭 |
| `Enter` | 在主列表或预览页直接使用原运行时恢复选中会话 |
...
| 文件 | 职责 |
|---|---|
| `sc.py` | 入口：curses TUI + 全屏聊天记录预览 + 显示列宽对齐 + 键位 + 拉起后台标题生成进程并轮询缓存刷新 + 回车接管终端；隐藏的 `--generate-titles` 模式即脱离终端的后台生成进程入口 |
