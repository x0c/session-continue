...
- 标题生成按「每批 5 条 × 最多 5 批并行」调用：25 条待生成会话会同时发起 5 个、每个处理 5 条的生成任务；更多会话在前一批完成后继续补齐。单批失败时保留该批临时标题，不逐条慢重试；每个成功批次完成即原子写缓存，界面可陆续显示结果。生成出的有效标题一旦写入缓存即为该会话的固定标题，后续对话内容增长不能让它再次排队；只有没有缓存或缓存标题无效时才生成。
