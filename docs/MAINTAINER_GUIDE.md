...
- **侧边栏卡片是固定的两行视觉契约**：第一行左侧“项目名: 标题”、右侧运行时；第二行左侧进程状态、右侧相对时间。运行中（含已托管）用绿色文字，已结束弱化；卡片下方再留一空行作为分隔，该空行画在卡片自身高度内（`SessionCard` 高 3 / `NewSessionCard` 高 2），算进本项命中区与选中高亮。标题生成中只在标题前轮换 spinner，不加粗标题——加粗会造成无意义的层级变化，在浅色终端还可能把 ANSI 前景色抬成低对比亮色。顶部搜索框必须显式使用可读前景色，不能依赖主题恰好给出正确颜色。
...
- **项目搜索（取代旧 `f` 循环筛选）**：侧边栏顶部常驻 `#project-search` 输入框；`nav.project_query` 为空表示不过滤。查询对项目名（cwd 末级）、完整路径和会话标题做大小写无关模糊匹配（子串或子序列，见 `pickup._fuzzy_match` / `_filter_sessions_by_query`）。列表聚焦时按 `/` 聚焦搜索框；搜索框内 Down/Enter 交回列表，Esc 先清空查询再交回列表（列表上 Esc 才退出）。新建会话目录：若当前查询命中恰好一个项目 cwd 则沿用，否则退回所选会话 cwd。
- 右栏默认随选择变化：托管会话显示 tmux 现场但不抢键盘；**未托管或已结束会话在右栏展示完整对话预览**（与旧 Space 全屏预览同款内容，选中即加载；走 `SessionStore.get_conversation` 缓存 + 后台 warm，不展示「加载中」；`EmbedPane` 用 `content-align: left top`，预览靠左靠上，不居中）。Enter 才恢复/操作，`p` 固定当前右栏后允许继续浏览其它会话。**键盘已交给面板（`ui.focus == "pane"`，Enter 明确聚焦）时不跟随**：新建/直启的空白会话没有列表会话键，跟随会立刻把刚聚焦的画面换回所选会话的静态详情——这是 `_follow_selection` 的第一条提前返回；焦点弹回列表（C-\、会话退出、面板被拖窄隐藏）后恢复跟随。
...
- **窄栏是卡片式多行布局**：`ui/session_list.py` 的 `SessionListView`（Textual `ListView`）里每个会话是一个高度固定为 3 行的 `SessionCard`：前两行是“项目: 标题”+ 运行时 / 状态 + 相对时间，第三行空行作分隔并算进命中区（不要用 `ListItem` 的 margin/padding 做分隔——点击落在空隙上不会选中项）。运行中状态为绿色，已结束弱化，生成中只加 spinner、不加粗。左栏 `LIST_PANE_WIDTH=39`；`SessionListView` 把滚动条占位收成 0（键盘/滚轮滚动仍可用）；长标题按显示宽度截断并加 `...`（`pickup._fit_cell(..., ellipsis=True)`），给右上角 runtime 名留完整空间。列表项间距、滚动、可见区域计算交给 Textual 原生处理，不再需要旧版手写的 `_draw`/`_sync_top`/`_scroll_list_viewport` 三处行数折算。顶部固定两行高的 `NewSessionCard`（虚拟索引 0，`NEW_SESSION_ID`，第二行空行同样可点），恒排第一，`rebuild()` 每次都优先 append 它。
