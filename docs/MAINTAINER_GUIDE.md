...
- **回滚上限的按需刷新（2026-07-19 用户实报「Codex 最多只能回滚三行」）**：`emb.history_size`（应用层回滚的上限）与鼠标标志平时只在 **pane 聚焦**时随抓帧刷新；列表聚焦下右栏跟随选择展示托管会话时这些值是陈旧快照（多为 0 或托管瞬间的几行），滚轮回滚被钉死。修复：`_handle_mouse` 在「从直播画面开始滚动」的一刻按需调一次 `pane_state` 刷新（0.3s 节流防止无历史 pane 上快速滚动风暴 fork）。回归验证脚本模式：托管一个「先打 1 行 → 停几秒 → 再补 300 行」的假会话，点击托管后 C-\ 回列表，等历史补完再在列表态对 pane 区域滚 3 格，断言出现「回滚 9 行」（v0.16.9 复现失败、修复版通过的负向对照已做）。另注：Codex/Claude/Kimi 实测都**不申请鼠标**（`mouse_any=0`）且不用备用屏，内嵌滚动全部依赖 tmux 历史 + 应用层 offset，SGR 转发分支目前无真实消费者。
