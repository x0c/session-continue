...
> **2026-07 界面层已从 curses 整体换成 Textual（详见「内嵌面板」节开头的迁移说明）。以下条目大多写于 curses 时代，描述 `_init_colors`/`PAIR_*`/`curses.mousemask`/`_draw_preview` 等已删除的具体机制，仅作历史存档，不代表当前 `ui/` 包的真实行为——本节尚未逐条重写核对，改动或验收界面行为前先看代码而不是假设这里的描述仍成立。** 下面三条是 Textual 版本里新发现、新修复的问题，写在最前面，不要和后面的 curses 时代记录混淆。

- **点击崩溃（2026-07 真机实报，已修复）**：Textual 默认给所有 Widget 开启内置的鼠标拖拽文本选择（`ALLOW_SELECT = True`）。`SessionCard`/`NewSessionCard` 这类会被后台重扫线程动态增删的列表项 Widget 被点击时触发该逻辑，选择过程中控件被移除会导致 `container` 解析为 `None`，访问 `.region` 直接 `AttributeError` 崩溃整个应用（Textual 自己的 issue 列表里能查到同类"widget 在选择过程中消失"的崩溃报告，不是 pickup 独有的边缘情况）。修法：`PickupApp`/`SessionCard`/`NewSessionCard`/弹窗菜单项 `_ChoiceItem` 设 `ALLOW_SELECT = False`；`EmbedPane`（画面不会被动态增删）保留 `ALLOW_SELECT` 默认值，用来实现划词选中+复制（见「内嵌面板」节）。`test_ui.py` 的 `test_clicking_session_card_selects_and_launches_without_crashing` 用 Pilot 模拟点击固化回归。
- **内嵌面板滚动卡顿（2026-07 真机实报，已修复）**：`EmbedPane` 最初的实现里，滚轮/resize 事件处理函数直接同步调用 `embed.capture()`/`embed.pane_state()`——这两个函数各自同步 fork 一个 tmux 客户端进程并阻塞等待返回，而这些事件处理函数跑在 Textual 的主 asyncio 事件循环上，每次滚动都会卡住整个 UI（快速滚动时尤其明显）。修法：抓帧改到专门的后台线程（`EmbedPane._capture_loop`，对应旧版 curses 的 `_capture_loop` 设计），`mouse_any`/`mouse_sgr`/`history_size` 等滚轮判断需要的状态改成后台线程写、主线程只读的缓存字段（`self._mouse_any` 等），滚轮/resize 事件处理函数不再发起任何阻塞调用，只更新 `history_offset` 并用 `threading.Event`（`self._poke`）唤醒后台线程尽快补抓一帧。
- **内嵌面板滚动卡顿第二波（2026-07-19 真机实报，v0.17.4 修复）**：上一波修复（后台抓帧线程）之后，滚动仍卡到整个界面冻结数秒。根因有三层：① 新版 Claude Code（v2.1.88 起默认全屏渲染）会在托管 pane 里申请 SGR 鼠标捕获——真机实测 2.1.214/2.1.215 全部 `mouse_any_flag=1`，旧结论「Claude 不申请鼠标、转发路径日常不生效」自此失效，滚轮改走「转发 SGR 序列给内层程序自己滚」路径；② `_wheel` 当时在 Textual 主线程上**同步** `send_literal(..., force_fork=True)` 两次——press + 一个 xterm 规范里根本不存在的滚轮 release 序列（规范明确滚轮按钮 64/65 只报 press、不报 release），每次 fork 约 10ms，触控板惯性滚动一次甩动产生几十上百个事件，主线程被连续阻塞数秒，表现是整个 pickup 冻结而不只是面板卡；③ Claude 每个滚轮事件都整屏重绘 → `%output` 风暴 → 抓帧循环以 25fps 上限跑，每帧 `capture-pane` + `display-message` 两次 fork，是 pickup 不滚动也常驻 40-64% CPU 的主要来源。修法：滚轮转发只发 press 序列，且改经 `embed.send_mouse_sequence` 排队到专用后台发送线程（限速 20ms，单会话积压超 12 条丢最旧——内层重绘不过来就跳中间帧，与 tmux 自身丢弃鼠标事件的策略一致），主线程零 fork；抓帧循环的 `pane_state` 查询降频到 5Hz（`STATE_POLL_INTERVAL=0.2`，光标/鼠标标志/回滚量都是慢变状态，IME 锚定最多滞后 200ms 无感知；查询失败也按同一节奏限速重试），画面文本仍按 `%output` 事件全速抓帧，输入回显帧率不受影响。回归测试：`test_embed.py` 的 `test_send_mouse_sequence_nonblocking_and_forked`/`test_send_mouse_sequence_queue_cap_drops_oldest`、`test_ui.py` 的 `EmbedPaneWheelTests`。注意 Claude 自己的全屏渲染滚动卡顿是上游已知问题（anthropics/claude-code#49624 等），pickup 侧只能消除转发与抓帧的放大效应。
- **会话列表刷新开销优化（2026-07-19）**：`SessionListView.rebuild()` 原来每次都 `clear()` 后逐项 `await self.append(...)`（每次 append 各一轮 mount/layout），每 3 秒后台重扫只要集合有任何变化就整表重建；`SessionCard.render()` 每张卡片各自调用一次 `store.snapshot()`（拿锁 + 拷贝 `display_titles` dict 和 `generating` set）；`_tick_spinner` 每 150ms 无条件 `snapshot()` 一次并遍历全部子项。修法三处：① `rebuild()` 新增「结构 vs 内容」判定——对比 `visible_sessions()` 返回的会话键序列和当前 `SessionListView` 子项对应的会话键序列，序列相同（无增删/顺序变化）时走 `_update_cards_in_place()`：只把 `SessionCard.session` 换成新 dict 引用、用 `_compute_signature()`（`display_title`/`is_generating`/`live`/`keepalive_name`/`mtime` 组成的元组）判断渲染相关字段是否真的变了，变了才 `refresh()`，完全不碰 `ListView` 子项结构；序列不同才走批量重建（`ListView.extend()` + `self.app.batch_update()` 包住 `clear()`+`extend()` 抑制中间重绘）。② `SessionCard` 不再自己调用 `store.snapshot()`——`display_title`/`is_generating` 改由外部（`rebuild()`/`_tick_spinner`）注入，构造时可选传入，避免多张卡片重复拷贝同一份 dict/set。③ `_tick_spinner` 先调用 `SessionStore.has_generating()`（只读一个 bool、不拷贝）判断有没有会话在生成标题，没有直接 return；有的话也只刷新命中 `generating` 的卡片。**`PreviewScreen` 已退役**（完整对话改由右栏 `EmbedPane.show_detail` / `_warm_conversation` 承担），不要再加回全屏预览页的条件刷新逻辑或相关测试；右栏详情依赖列表 `rebuild` 时的 `invalidate_detail()` + 会话键稳定解析，不能继续展示闭包捕获的旧 dict。**Textual `clear()`+`extend()` 后设置 `index` 高亮不刷新的已知问题（GitHub issue #6300）**：当前锁定 Textual ≥8.0，Pilot 探查未稳定复现，但 `rebuild()` 批量重建路径末尾仍保留防御性 `self.refresh()`。**教训**：只断言 `list_view.index`/`card.session` 这类内部状态不能证明渲染结果对——`test_rebuild_updates_in_place_when_session_set_unchanged` 额外断言 `card.render().plain` 含预期状态文案。回归测试：`test_ui.py` 的 `test_rebuild_updates_in_place_when_session_set_unchanged`/`test_rebuild_falls_back_to_full_rebuild_when_session_set_changes`/`test_tick_spinner_skips_snapshot_when_nothing_generating`/`test_tick_spinner_refreshes_only_generating_cards`。
- **Textual 后台 worker 必须可取消**：初始加载等待用短超时轮询检查 `get_current_worker().is_cancelled`；会话重扫循环用 `worker.cancelled_event.wait(interval)` 同时承担定时与取消唤醒，禁止回退到无限 `while True + time.sleep()` 或无期限 `Event.wait()`——Screen 卸载、`run_test()` 退出时 Textual 只能发取消信号，线程不观察信号就会永远拖住应用/测试。worker 内不得直接读写 Widget/DOM；主线程先取尺寸等布局状态，后台只做阻塞 I/O，结果经 `app.call_from_thread` 回写。后台启动会话同样走单飞 worker，避免 tmux 最长数秒的创建等待冻结事件循环。
- **侧边栏末行间隔（硬约定，凡往左栏加控件都必须遵守）**：每个块的最后一行是间隔空行，画在控件自身高度内并算进命中区与选中高亮；**禁止**用 `margin`、兄弟节点空隙或 `ListItem` 的 padding/margin 做分隔——点在空隙上不会选中本项（真机反馈后改为控件内空行）。当前基准：`#project-search` 高 2（正文 1 + 间隔 1）、`NewSessionCard` 高 2、`SessionCard` 高 3（两行正文 + 间隔 1）。会话卡正文契约不变：第一行左侧“项目名: 标题”、右侧运行时；第二行左侧进程状态、右侧相对时间。运行中（含已托管）用绿色文字，已结束弱化。标题生成中只在标题前轮换 spinner，不加粗标题——加粗会造成无意义的层级变化，在浅色终端还可能把 ANSI 前景色抬成低对比亮色。顶部搜索框必须显式使用可读前景色，不能依赖主题恰好给出正确颜色。
- **（历史）`project_key` 状态重复、两处不同同步**：旧版 `f` 循环筛选时，`SessionListView` 与 `MainScreen.nav` 曾各存一份 `project_key` 导致页头文案和过滤结果脱节；已改为共用 `nav`。2026-07-19 起 `f` 循环已退役，改为顶部搜索框 + `nav.project_query`，但「筛选状态只认 nav 一份、测试必须断言渲染结果」这条教训仍然有效。

- TUI 表格/卡片列宽必须按终端显示宽度计算，统一走 `rich.cells.cell_len`（经 `pickup._text_width` / `_fit_cell`），不要用 Python 字符数直接 `ljust` 或切片，也不要另维护一份 wcwidth 表；中文、箭头和状态图标会占 2 个显示列。
...
- **已修复（`q` 结束会话确认框曾在 200ms 后自动消失）**：`_confirm_kill_keepalive` 原实现只调用一次 `stdscr.getch()`，但 `stdscr` 处于 `_run` 设的 200ms 非阻塞超时模式，超时返回 `-1` 曾被当成"其他键取消"处理——确认框实际只有 200ms 窗口可按 `y`，之后就自动消失，用户几乎不可能在这个窗口内按到确认键。修法：改成循环等待，`getch()` 返回 `-1`（超时）就继续等，只有真正读到按键（含 `curses.error` 之外的任何返回值）才判定 `y`/`Y` 是否命中。新增任何在 200ms 超时模式下需要"等待用户单次按键确认"的弹窗，都要走这个循环写法，不要直接信任裸 `getch()` 一次调用的返回值。
- **已修复（终端被拖窄导致内嵌面板隐藏后，按键仍被转发进看不见的会话）**：`_run` 主循环里 `emb.dead and ui.focus == "pane"` 只处理了"面板里的会话进程已退出"这一种情况，把焦点弹回列表；但终端被拖窄导致 `_embed_list_width` 返回全宽（面板本身不可见，会话在后台 tmux 里仍然活着）是另一种情况，之前没有对应处理——此时界面显示的是全宽列表，但 `ui.focus` 仍是 `"pane"`，所有按键（包括 `Esc`）都会被 `_forward_pane_key` 转发进这个已经看不见的托管会话，表现像键盘失灵，只能靠 `C-\` 逃生。修法是同一处主循环新增 `elif width == full_width and ui.focus == "pane"` 分支，面板隐藏时同样把焦点强制拉回列表；拉宽终端后用户可以再次回车重新聚焦面板。
...
- **踩坑（2026-07-19）：按 `n` 新建闪退**——`_host_and_focus` 成功回调 `_on_embed_hosted` 同时服务 `LaunchRequest`（恢复/接力，有 `.session`）和 `NewSessionRequest`（空白新建，只有 `target_runtime_id`/`cwd`）。曾无条件读 `request.session`，新建路径立刻 `AttributeError` 崩掉。收尾里凡要碰会话对象，必须先 `isinstance(..., LaunchRequest)`；空白新建只 `focus_session(name)`、不写 hosted 标注。回归：`test_ui.MainScreenHostWorkerTests.test_new_session_shortcut_hosts_without_reading_session`。
- 列表固定两行正文 + 一行分隔空行：第一行左侧“项目名: 标题”、右侧运行时；第二行左侧状态、右侧相对时间；第三行空行属本卡可点区域。运行中（含已托管）用绿色文字，已结束弱化。生成中只在标题前显示 spinner，不改变标题字重。项目名从完整 cwd 取末级目录；顶部搜索框按模糊查询过滤可见会话（见「项目搜索」）。
- **项目搜索（取代旧 `f` 循环筛选）**：侧边栏顶部常驻 `#project-search`（高 2：一行输入 + 末行间隔，遵守侧边栏末行间隔约定）；`nav.project_query` 为空表示不过滤。查询对项目名（cwd 末级）、完整路径和会话标题做大小写无关模糊匹配（子串或子序列，见 `pickup._fuzzy_match` / `_filter_sessions_by_query`）。列表聚焦时按 `/` 聚焦搜索框；搜索框内 Down/Enter 交回列表，Esc 先清空查询再交回列表（列表上 Esc 才退出）。新建会话目录：若当前查询命中恰好一个项目 cwd 则沿用，否则退回所选会话 cwd。
- 右栏默认随选择变化：托管会话显示 tmux 现场但不抢键盘；**未托管或已结束会话在右栏展示完整对话预览**（与旧 Space 全屏预览同款内容，选中即加载；走 `SessionStore.get_conversation` 缓存 + 后台 warm，不展示「加载中」；`EmbedPane` 用 `content-align: left top`，预览靠左靠上，不居中）。Enter 才恢复/操作。长对话用 `Home`/`End`/`PgUp`/`PgDn` 或右栏上的滚轮翻看（`EmbedPane.detail_offset`，勿命名为 `scroll_offset`）。**键盘已交给面板（`ui.focus == "pane"`，Enter 明确聚焦）时不跟随**：新建/直启的空白会话没有列表会话键，跟随会立刻把刚聚焦的画面换回所选会话的静态详情——这是 `_follow_selection` 的第一条提前返回；焦点弹回列表（C-\、会话退出、面板被拖窄隐藏）后恢复跟随。
- Space 全屏预览页（`PreviewScreen`）已退役；完整对话只由右栏承担。
...
- `n`（新建空白会话）：工作目录由 `pickup._new_session_cwd` 解析——当前搜索若恰好命中一个项目则优先，否则退回光标所在会话的 `cwd`；两者都拿不到，或解析出的目录在本机不存在（`usable_cwd` 校验），一律蜂鸣后停留，不弹窗、不报错。运行时直接用当前来源，不弹窗；需要选项目/运行时时走列表顶部固定的「＋ 新建会话」项（见「统一会话时间线与右栏跟随」节），完整选择流程只有那一个入口。
...
> **界面层迁移说明（2026-07）**：界面已从手写 curses 整体换成 [Textual](https://github.com/Textualize/textual)，`embed.py` 的 tmux 抓帧/输入转发/控制通道仍保持 UI 框架无关的架构边界，但其性能与生命周期实现会继续演进。旧版 `pickup.py` 的 `_run` 主循环 + `_draw_embed_pane` 被 `ui/main_screen.py` 的 `MainScreen` + `ui/embed_pane.py` 的 `EmbedPane` widget 取代；`PairPool`（curses 颜色对池）被框架中立的 `embed.cell_style(cell) -> rich.style.Style` 取代；`translate_key(curses键码)` 被 `translate_textual_key(key字符串)` 取代。下文凡是描述 curses/ncurses 内部行为的部分仅作历史存档；tmux/协议层结论仍适用，但以同节较新的 ControlChannel、Line API 和滚动约束为准。**鼠标拖拽跨行选词 + 复制**直接复用 Textual 文本选择：拖动高亮，`Ctrl+C` 经 OSC 52 复制；`Ctrl+C` 判断必须留在 `EmbedPane._on_key`，因为 widget `event.stop()` 后 Screen `BINDINGS` 不会再执行。无选区时 `Ctrl+C` 继续转发给托管会话中断命令。

- **定位**：与 `keepalive.py` 平级的运行时无关层。keepalive 管「把启动计划包进 tmux 保活」，embed 管「不 attach——用 `capture-pane` 拿画面、`send-keys` 送按键」，让 TUI 回车后退化成左侧会话列表（固定 ~39 列，`ui/main_screen.py` 的 `LIST_PANE_WIDTH`）+ 右侧会话现场（`EmbedPane`）。与保活共用 `tmux -L pickup-keepalive` socket 和 `pickup-*`/`sc-*` 命名空间：`e` 键全屏接管（execvp attach）、`keepalive.annotate()` 状态标注、`reap_idle()` 空闲回收、`q` 结束进行中会话，对内嵌会话全部照旧生效。适配器不感知本模块。
- **窄栏是卡片式多行布局**：遵守「末行间隔」硬约定（见 AGENTS.md / 上文）。`SessionListView` 里每个会话是高度 3 的 `SessionCard`（两行正文 + 末行间隔）；`NewSessionCard` 高 2；`#project-search` 高 2。间隔画在控件自身内并算进命中区，不要用 `ListItem` 的 margin/padding。运行中状态为绿色，已结束弱化，生成中只加 spinner、不加粗。左栏 `LIST_PANE_WIDTH=39`；`SessionListView` 把滚动条占位收成 0；长标题按显示宽度截断并加 `...`，给右上角 runtime 名留完整空间。
- **runtime 名配色（单一来源）**：色表与样式串在 `pickup.py` 的 `RUNTIME_LABEL_STYLES` / `runtime_label_style(runtime_id)`（按 runtime **id**，不是 display_name）。左栏 `SessionCard`、右栏详情头、对话预览里的 `◆ Runtime` 行必须共用这一处，禁止再在 `ui/` 里另写一份 hex。配色优先一眼可辨、不强制品牌色复刻——Cursor 品牌橙与 Claude 撞色，故 Cursor 用紫：`claude=#D97757`、`codex=#60A5FA`、`cursor=#A78BFA`、`kimi=#F472B6`、`opencode=#34D399`，未知回退 `dim`；展示用 `bold <color>`。新增 runtime 只加色表一行。
- **踩坑（2026-07-19）：改了配色但 TUI 仍无色**——`~/.local/bin/pickup` 入口加载的是 user site-packages 里的安装副本，**不随 `cli/` 源码目录自动更新**；在 `cli/` 下 `import pickup` / 跑单测看到的是源码（有色），用户直接敲 `pickup` 仍可能是旧包（无色）。`pip show pickup` 的 Version 跨 Python 版本还可能误导。界面改完验收前必须确认入口与色表一致：`python3 -c "import pickup; print(pickup.__file__); print(pickup.runtime_label_style('claude'))"` 应打印 site-packages 或当前 `cli/pickup.py`，且样式为 `bold #D97757`；否则执行 `python3 -m pip install --user --force-reinstall --no-deps .`（本机 `pip install -e .` 曾因 setuptools/easy_install 权限失败，非 editable 的 force-reinstall 更稳），然后**重启**已打开的 TUI。Textual `save_screenshot` 导出的 SVG 常把真彩色压成灰阶，**不能**靠 SVG fill 判断 runtime 配色是否生效——用真机 TUI 或 Pilot 下 `SessionCard.render_line(0)` 的 segment style 核对。
...
- **终端背景色注入（深/浅主题检测修复）**：tmux 对 pane 内的 OSC 11 背景色查询的应答取决于有无 client——无 client 时石沉大海（查询超时），有 client（内嵌场景恒有控制 client）时按 client 默认值**应答黑色**，agent 因此在浅色终端上被误判成深色主题（这不是内嵌引入的，全屏 attach 一样）。`main()` 趁 Textual 接管终端前（`ui.app.run_app()` 之前，`_probe_osc_colours()` 本身不依赖 curses，未随迁移改动）`_probe_osc_colours()` 向外层终端查询 OSC 10/11 拿应答原文（非 TTY/不应答则 None；测试钩子 `PICKUP_OSC_REPORT`=hex）。
  - **`pickup` 自身界面的深浅色**是另一件独立的事：`PickupApp.on_mount` 用 `pickup._background_is_light(osc_report)`（解析 OSC 11 的 `rgb:RRRR/GGGG/BBBB`，ITU-R BT.709 亮度公式，阈值 0.5）决定 `self.theme = "textual-light"` 还是 `"textual-dark"`——这条 2026-07 才补上，之前完全没接，Textual 默认主题在浅色终端下配色不对（真机反馈）。
...
- **Textual 版本鼠标处理现状**：`EmbedPane` 自己只处理 `events.MouseScrollUp`/`MouseScrollDown` 两种滚轮事件（`ui/embed_pane.py` 的 `_on_mouse_scroll_up`/`_on_mouse_scroll_down` → `_wheel`）。自然方向必须固定为 Up 增加 `history_offset`（进入更早历史）、Down 减少（回到直播）；这个应用层整数**不得命名为 `scroll_offset`**，后者是 Textual `Widget` 的内置二维 `Offset`，覆盖后会让框架选区坐标执行 `Offset + int` 崩溃。直播位置且 pane 内程序申请鼠标上报时，仍转成 SGR **press-only**：64=上滚、65=下滚，经 `embed.send_mouse_sequence` 排队发送；已经进入历史或程序未申请鼠标时才更新 `history_offset`。点击、拖拽由 Textual 自己路由；改动或验收前先确认是在 Textual 事件层还是 `embed.py` 的 tmux 协议层。
...
```bash
python3 -m py_compile pickup.py scan_claude.py scan_codex.py scan_opencode.py scan_kimi.py scan_cursor.py scan_common.py titles.py titlegen.py models.py agent_api.py keepalive.py embed.py runtime/*.py test_*.py
