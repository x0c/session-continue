...
> **界面层迁移说明（2026-07）**：界面已从手写 curses 整体换成 [Textual](https://github.com/Textualize/textual)，后端逻辑（`embed.py` 的 tmux `capture-pane`/`send-keys`/`ControlChannel`/`parse_screen`）完全没变——这一层本来就与 UI 框架无关。变的只是「谁调用它、怎么画出来」：旧版 `pickup.py` 的 `_run` 主循环 + `_draw_embed_pane` 被 `ui/main_screen.py` 的 `MainScreen` + `ui/embed_pane.py` 的 `EmbedPane` widget 取代；`PairPool`（curses 颜色对池）被框架中立的 `embed.cell_style(cell) -> rich.style.Style` 取代，`embed.grid_to_text(grid)` 把画面转成 Rich `Text` 交给 Textual 渲染；`translate_key(curses键码)` 被 `translate_textual_key(key字符串)` 取代（Textual 按键事件是稳定字符串如 `"up"`/`"ctrl+c"`，不再是 curses 平台相关的整数键码）。下文按主题记录的具体机制、坑和实测结论，凡是描述 curses/ncurses 内部行为（颜色对分配、`mousemask`/`getmouse`、SGR 1006 协商、macOS `BUTTON5_PRESSED` 缺失等）的部分，对应代码已随迁移整体删除，仅作历史存档；凡是描述 tmux/协议层行为（控制通道、抓帧去重、光标锚定、主题注入根因）的部分，在 Textual 版本里原样成立。**鼠标拖拽跨行选词 + 复制（2026-07 补齐，不再是已知限制）**：`EmbedPane` 没有手写 `sel_anchor`/`chgat` 那套选区逻辑，而是直接复用 Textual 内置的鼠标拖拽文本选择（不设 `ALLOW_SELECT = False`）——按下拖动即可高亮选中画面里的文字，`Ctrl+C` 复制到剪贴板（`app.copy_to_clipboard`，即 OSC 52，真机在 tmux 里用 `set-clipboard on` + `show-buffer` 验证过真实写入落地；文档同 macOS Terminal.app 不支持 OSC 52 的限制）。**为什么 `Ctrl+C` 的判断逻辑必须写在 `EmbedPane._on_key` 里、不能用 Textual 的 `BINDINGS`**：真机验证过 Textual 的按键派发是「事件先转发到当前聚焦 widget 的 `on_key`，widget 自己处理并 `event.stop()` 掉之后，`BINDINGS`（包括 `Screen` 基类自带的 `ctrl+c -> copy_text`）根本没有机会再介入」——不是「更具体的 widget 绑定优先于 Screen 绑定」那套（曾经真的写过一版基于 `BINDINGS` 的实现，Pilot 测试证明它是死代码，从未被调用到）。`_on_key` 里判断：有选中文本就复制（复刻 `Screen.action_copy_text`），没有就转发 `C-c` 给托管会话中断当前命令——两者都用真实 tmux + `trap INT`/`show-buffer` 验证过。
...
- **渲染为什么不自己写终端模拟器**：`capture-pane -p -e` 输出的就是 tmux（它本身就是终端模拟器）渲染好的当前画面加 SGR 颜色序列，`embed.parse_screen` 只需一个 SGR 状态机 + wcwidth 落格，真彩色量化到 256 色。curses 端颜色对不可预知，曾用 `PairPool`（LRU 分配 `init_pair` 编号，静态颜色对 1-15 之后从 16 起）按需分配、池满先丢背景色再放弃颜色；**该问题在 Textual 版本里已不存在**——`embed.cell_style(cell)` 直接把每个格子的颜色/属性转成 `rich.style.Style` 对象，`embed.grid_to_text(grid)` 拼成带 spans 的 Rich `Text` 交给 Textual 渲染，颜色数量不受 curses 颜色对（256 个）上限约束，不需要任何分配/回收池。物理重绘去重此前交给 ncurses `refresh()` 的内部 diff，现在是 Textual `Widget.refresh()` 自己的脏区机制，两者效果等价。
...
- **（历史）向下回直播的跨终端兼容规则（2026-07 用户真机确认）**：不能把 `BUTTON2_PRESSED` 当作唯一根因或唯一修复；它只是旧版 macOS ncurses 的一个已知伪装形式，触控板和不同终端还会产生无法可靠枚举的 `KEY_MOUSE` 状态组合。标准 Button5、已知 Button2 都可以优先识别；但只要已经处于回滚（`scroll_offset > 0`），任何**非左键**鼠标事件都按“向下回直播”处理，左键事件保留给选词。这一状态优先的降级规则才保证了先上滚、再用鼠标或触控板下滚能够回到直播；不要退回到“把所有鼠标事件转交内层 TUI”或继续猜测某一个位掩码。验收必须在真实终端完成“向上进入历史 → 鼠标/触控板向下 → 回到直播”的完整路径；只确认 SGR 序列能到达内层程序，不构成验收。

- **Textual 版本鼠标处理现状**：`EmbedPane` 自己只处理 `events.MouseScrollUp`/`MouseScrollDown` 两种滚轮事件（`ui/embed_pane.py` 的 `_on_mouse_scroll_up`/`_on_mouse_scroll_down` → `_wheel`），逻辑与旧版保留的核心判断一致——pane 内程序确实申请了鼠标上报（`pane_state()` 的 `mouse_any` 位）时转成 SGR **press-only** 序列、经 `embed.send_mouse_sequence` 排队到后台线程发送（主线程零 fork；滚轮无 release 事件；积压丢最旧——设计与根因见上方「滚动卡顿第二波」条目），否则退化为 pickup 自己的应用层回滚（`_scroll`，见下方「滚轮翻历史」条目，机制不变）。点击、拖拽由 Textual 自己路由，不需要 pickup 手写坐标判断；跨行拖拽选词见下一条（2026-07 已用 Textual 内置机制补齐，不再是限制）；状态优先的「非左键回直播」判断随 curses 手写鼠标协议一起删除，不适用于 Textual。改动或验收鼠标相关行为前，先确认是在 Textual 事件层还是 `embed.py` 的 tmux 协议层。
