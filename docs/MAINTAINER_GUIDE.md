...
- **侧边栏末行间隔（硬约定）**：每个块最后一行是间隔空行，画在控件自身高度内并算进命中区；禁止用 margin/兄弟空隙/`ListItem` padding。基准：搜索框高 2、新建项高 2、会话卡高 3。
...
- `SessionStore.all_sessions()` 合并全部运行时后按 `_order` 稳定顺序：首次按 mtime 倒序，之后已有项位置固定，新会话按 mtime 插最前。
- 列表虚拟索引 0 是顶部固定「＋ 新建会话」；默认选中最近会话。该项回车走 `new_session_flow`；`n` 是不弹窗快捷路径。
- **踩坑：按 `n` 新建闪退**——托管成功回调必须区分 `LaunchRequest` / `NewSessionRequest`，空白新建禁止读 `.session`。回归：`test_new_session_shortcut_hosts_without_reading_session`。
- 会话卡两行正文 + 末行间隔；运行中绿色、已结束弱化；生成中只加 spinner。
- **项目搜索**：`#project-search` + `nav.project_query`；`/` 聚焦搜索；Down/Enter 回列表；Esc 先清空再回列表。
- 右栏随选择变化：托管显示现场；未托管/已结束显示完整对话预览（选中即加载）。面板聚焦时不跟随列表。长对话用 Home/End/PgUp/PgDn 或滚轮（`detail_offset`）。
- 点击会话卡等价 Enter；右栏聚焦时 `Ctrl-\` 交回列表。

### 会话级快捷键

- `a`/`n`/`q`/`e` 等会话动作集中在 `MainScreen` 与 `ui/modals.py`；不要再拆第二套「预览页专用」按键分发。
- `n`：cwd 由 `_new_session_cwd` 解析（唯一命中项目优先，否则所选会话 cwd）；无效则蜂鸣停留。
- `a`：无具体会话时只 beep。
