...
- **回收（`reap_idle`）**：按 tmux 自己维护的 `#{session_activity}`（该会话最后一次有任何活动的时间戳）判断空闲时长，超过 `PICKUP_KEEPALIVE_IDLE_HOURS`（旧名 `SC_KEEPALIVE_IDLE_HOURS` 仍生效；默认 6，`0` 禁用）就 `kill-session`。不常驻额外的守护进程/定时器——`main()` 在进 TUI 前顺带跑一次，随 `pickup` 的启动节奏自然触发，足够覆盖"长期没人用 pickup 就不会占着内存"的诉求；会话历史本身在磁盘上，回收只是关掉后台进程，不丢数据。
