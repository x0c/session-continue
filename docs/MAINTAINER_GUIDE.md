...
- **会话列表/预览页刷新开销优化（2026-07-19）**：`SessionListView.rebuild()` 原来每次都 `clear()` 后逐项 `await self.append(...)`（每次 append 各一轮 mount/layout），每 3 秒后台重扫只要集合有任何变化就整表重建；`SessionCard.render()` 每张卡片各自调用一次 `store.snapshot()`（拿锁 + 拷贝 `display_titles` dict 和 `generating` set）；`_tick_spinner` 每 150ms 无条件 `snapshot()` 一次并遍历全部子项；`PreviewScreen._poll_updates` 每 1 秒无条件重新折行 + 重建 `Text` + `Static.update()`（全量 re-layout），哪怕内容压根没变。修法三处：① `rebuild()` 新增「结构 vs 内容」判定——对比 `visible_sessions()` 返回的会话键序列和当前 `SessionListView` 子项对应的会话键序列，序列相同（无增删/顺序变化）时走 `_update_cards_in_place()`：只把 `SessionCard.session` 换成新 dict 引用、用 `_compute_signature()`（`display_title`/`is_generating`/`live`/`keepalive_name`/`mtime` 组成的元组）判断渲染相关字段是否真的变了，变了才 `refresh()`，完全不碰 `ListView` 子项结构；序列不同才走批量重建（`ListView.extend()` + `self.app.batch_update()` 包住 `clear()`+`extend()` 抑制中间重绘）。② `SessionCard` 不再自己调用 `store.snapshot()`——`display_title`/`is_generating` 改由外部（`rebuild()`/`_tick_spinner`）注入，构造时可选传入，避免多张卡片重复拷贝同一份 dict/set。③ `_tick_spinner` 先调用新增的 `SessionStore.has_generating()`（只读一个 bool、不拷贝任何东西）判断有没有会话在生成标题，没有直接 return、连 `snapshot()` 都不调；有的话也只刷新 `session_key` 命中 `generating` 集合的那几张卡片。④ `PreviewScreen._render_messages()` 新增 `force` 参数，轮询（`_poll_updates`）时先比较本次 `get_conversation()` 返回的消息列表和折行宽度是否都和上次渲染时相同，都相同才跳过折行+重建+`Static.update()`；**隐性依赖**：条件刷新会漏掉"窗口 resize 但内容没变"这种情况（折行宽度是按当前终端宽度算的），因此新增 `PreviewScreen._on_resize` 钩子，resize 时强制 `_render_messages(force=True)` 重新折行，这是之前完全没有的功能缺口。**Textual `clear()`+`extend()` 后设置 `index` 高亮不刷新的已知问题（GitHub issue #6300，报告环境 Textual 6.11.0）**：项目当前锁定版本是 8.2.8（`pyproject.toml` `textual>=8.0`），用 Pilot 直接探查过 `Compositor.render_update(full=False)` 返回的增量脏区，在多种时序下都没有复现出"`ListItem.highlighted`/`-highlight` CSS class 已经正确设置、但需要额外交互才刷新到屏幕"的现象——但探查手段本身有局限（无法完全模拟真实终端的分帧渲染时序，`export_screenshot()`/`render_update(full=True)` 这类强制全量渲染的检测方式天生测不出这类问题）。鉴于无法确证复现，同时补一次显式 `self.refresh()` 成本几乎为零，`rebuild()` 的批量重建路径末尾保留了这个防御性调用兜底，不依赖"确认没有这个坑"才决定要不要修。**教训**：只断言 `list_view.index`/`card.session` 这类内部状态不能证明渲染结果对——`test_ui.py` 新增的 `test_rebuild_updates_in_place_when_session_set_unchanged` 额外断言了 `card.render().plain` 里包含预期的状态文案，同上面 `project_key` 那条教训一致。回归测试：`test_ui.py` 的 `test_rebuild_updates_in_place_when_session_set_unchanged`/`test_rebuild_falls_back_to_full_rebuild_when_session_set_changes`/`test_tick_spinner_skips_snapshot_when_nothing_generating`/`test_tick_spinner_refreshes_only_generating_cards`/`test_poll_skips_static_update_when_content_and_width_unchanged`/`test_resize_forces_rerender_with_new_wrap_width_even_if_content_unchanged`。
...
- **侧边栏卡片是固定的两行视觉契约**：第一行左侧“项目名: 标题”、右侧运行时；第二行左侧进程状态、右侧相对时间。运行中（含已托管）用绿色文字，已结束弱化；卡片之间留一空行。标题生成中只在标题前轮换 spinner，不加粗标题——加粗会造成无意义的层级变化，在浅色终端还可能把 ANSI 前景色抬成低对比亮色。页头必须显式使用可读前景色，不能依赖主题恰好给出正确颜色。
- **`project_key` 状态重复、两处不同步（2026-07 真机跑 `selftest.sh` 才暴露，headless 测试没拦住）**：`SessionListView` 曾经自己维护一份 `self.project_key`（供 `visible_sessions()`/`cycle_project_filter()` 过滤用），`MainScreen.nav.project_key` 是另一份（供 `_update_header()`/`_new_session_cwd` 等跨组件逻辑用）——`f` 键筛选时只更新了前者，后者永远停在 `None`，导致页头文案（"全部项目"）和实际过滤结果（真的按项目筛过了）对不上。修法：`SessionListView.__init__` 改为接收 `nav` 引用（`SessionListView(store, nav, ...)`），`visible_sessions()`/`cycle_project_filter()` 一律读写 `self.nav.project_key`，不再自己另开一份状态。**教训**：`test_ui.py` 最初的回归测试只断言了 `list_view.project_key`（内部状态），没有断言页头渲染出的文本，因此没拦住这个 bug——凡是"内部状态 A 应该驱动渲染结果 B"的场景，测试必须同时断言 B（渲染文本/DOM 属性），只断言 A 不能证明两者同步。
...
- 列表固定两行卡片：第一行左侧“项目名: 标题”、右侧运行时；第二行左侧状态、右侧相对时间。运行中（含已托管）用绿色文字，已结束弱化；卡片之间留一空行。生成中只在标题前显示 spinner，不改变标题字重。项目名从完整 cwd 取末级目录；筛选匹配仍必须按归一化后的完整 cwd，不能按展示名匹配。
...
- **定位**：与 `keepalive.py` 平级的运行时无关层。keepalive 管「把启动计划包进 tmux 保活」，embed 管「不 attach——用 `capture-pane` 拿画面、`send-keys` 送按键」，让 TUI 回车后退化成左侧会话列表（固定 ~44 列，`ui/main_screen.py` 的 `LIST_PANE_WIDTH`）+ 右侧会话现场（`EmbedPane`）。与保活共用 `tmux -L pickup-keepalive` socket 和 `pickup-*`/`sc-*` 命名空间：`e` 键全屏接管（execvp attach）、`keepalive.annotate()` 状态标注、`reap_idle()` 空闲回收、`x` 关闭后台，对内嵌会话全部照旧生效。适配器不感知本模块。
- **窄栏是卡片式多行布局**：`ui/session_list.py` 的 `SessionListView`（Textual `ListView`）里每个会话是一个高度固定为 2 行的 `SessionCard`：第一行“项目: 标题”+ 右对齐运行时，第二行状态 + 右对齐相对时间；`ListItem` 再提供一行下边距分隔卡片。运行中状态为绿色，已结束弱化，生成中只加 spinner、不加粗。列表项间距、滚动、可见区域计算交给 Textual 原生处理，不再需要旧版手写的 `_draw`/`_sync_top`/`_scroll_list_viewport` 三处行数折算。顶部固定一行高的 `NewSessionCard`（虚拟索引 0，`NEW_SESSION_ID`），恒排第一，`rebuild()` 每次都优先 append 它。
...
- **渲染为什么不自己写终端模拟器**：`capture-pane -p -e` 输出的就是 tmux（它本身就是终端模拟器）渲染好的当前画面加 SGR 颜色序列，`embed.parse_screen` 只需一个 SGR 状态机 + wcwidth 落格，真彩色量化到 256 色。curses 端颜色对不可预知，曾用 `PairPool`（LRU 分配 `init_pair` 编号，静态颜色对 1-15 之后从 16 起）按需分配、池满先丢背景色再放弃颜色；**该问题在 Textual 版本里已不存在**——`embed.cell_style(cell)` 直接把每个格子的颜色/属性转成 `rich.style.Style` 对象。实时画面使用 Textual Line API：按行缓存 `Strip`、比较新旧 `Cell` 行，只重编译并刷新变化行；`render()` 仅作框架内部/既有测试的兼容入口，必须复用 `render_line()` 结果，不能再维护第二套渲染逻辑。
