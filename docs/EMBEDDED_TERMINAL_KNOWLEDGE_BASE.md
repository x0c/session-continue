...
内嵌实时终端把“运行中(托管)”的助手会话画面放进终端界面的右栏，让用户保留会话列表、同时看见并操作正在运行的助手。用户可按 `e` 全屏接管；这不是另一套会话或终端模拟器，而是同一份后台 tmux 会话的不同观看方式。
...
- **全屏**（`action.fullscreen`、快捷键 `e`）：直接 attach 已托管会话，由 tmux 全屏接管终端；适合需要完整鼠标能力或不适合右栏的场景。
...
- 普通字符经 `send_literal()` 原样发送；特殊键经 `translate_textual_key()` 转成 tmux 键名后由 `send_key()` 发送。
...
- 鼠标拖拽选词与复制由 Textual 的原生文本选择完成；本版不把点击、拖拽操作转发给内层程序。需要直接操作内层鼠标时用全屏。
...
| `ui/main_screen.py` | 终端界面挂接、异步托管启动、全屏和关闭分栏 | `MainScreen._embed_open()`、`_on_embed_hosted()`、`action_fullscreen()` |
| `pickup.py` | 启动接线、tmux 硬依赖检查、外层背景色探测 | `_require_tmux()`、`_probe_osc_colours()` |
...
| 打开或接回右栏会话 | `ui/main_screen.py` | `MainScreen._embed_open()`、`MainScreen._on_embed_hosted()` | 在后台完成阻塞创建，成功后聚焦内嵌实时终端 |
| 聚焦实时画面 | `ui/embed_pane.py` | `EmbedPane.focus_session()` | 切会话、开控制通道、调整尺寸、启动首帧抓取 |
...
| 常规输入 | `ui/embed_pane.py` | `EmbedPane._on_key()`、`_on_paste()` | 文本、特殊键、Ctrl+C、Ctrl+\ 和粘贴的用户语义分流 |
| 鼠标滚轮与历史 | `ui/embed_pane.py` | `EmbedPane._wheel()`、`_scroll()` | 转发鼠标或变更应用层回滚偏移，主线程不阻塞 |
...
| 全屏接管 | `ui/main_screen.py` | `MainScreen.action_fullscreen()` | `e` 进入 tmux attach；继续使用同一后台会话 |
...
| 托管会话 | `pickup-*`、`sc-*` | `embed.host_session()`、`keepalive._session_name()` | 创建、接回、全屏、标注与回收同一会话 |
...
- **AI 易错点**【隐性依赖】内嵌实时终端与会话保活必须共用专用 socket 和 `pickup-*` / `sc-*` 命名空间，否则已托管会话无法被正确接回、标注、全屏或回收。
...
- 【消歧】**全屏**是用户按 `e` attach 同一托管会话；**关闭分栏**只隐藏内嵌实时终端并让会话继续运行；**结束会话**才会停止运行中的助手。这三个操作不能互相替代。
...
3. 用户验收必须进入真实终端界面：打开一个运行中(托管)会话，确认右栏无“连接中…”卡死；输入可即时到达助手；上滚可查看历史、下滚可回到直播；`Ctrl+\` 回列表不杀会话；`e` 能全屏接管；会话自然结束后右栏回退为结束态。
...
5. 按 `e` 全屏接管同一会话；退出全屏后再次进入 pickup，确认仍能接回。
6. 让测试会话自然退出，确认右栏安全回退。任何真实用户会话都不得为了测试而被杀掉。

## §8 关联文档

- `docs/MAINTAINER_GUIDE.md`：改、评审或排查内嵌面板、会话保活、控制通道、输入延迟、滚动、主题、IME、tmux 冒烟时联读；这是会话保活策略和真实踩坑的权威维护说明。
- `docs/TERMINAL_UI_KNOWLEDGE_BASE.md`：涉及右栏与列表焦点、全屏动作、静态对话预览、界面事件路由时联读；该文档覆盖终端界面布局与交互边界，本知识库不重复其内容。
...
- 领域语言统一：主称谓为“内嵌实时终端”“运行中(托管)”“会话保活”“全屏”；旧 `embed` / `EmbedPane` / `capture-pane`、hosted / `host_session`、`SC_*` 仅作为定位别名保留。
...
| 内层鼠标点击 / 拖拽转发尚未实现 | 高 | 当前事件处理边界和维护指南 | 产品需要时先定义点击、拖动、选择与全屏的冲突规则 |
