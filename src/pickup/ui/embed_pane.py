...
docstring），取代旧版 curses 手写的框选高亮 + OSC 52 复制。划词抬起由
MainScreen.on_text_selected 自动 copy_to_clipboard（OSC 52）。

Ctrl+C 语义特别说明：抬起已自动复制后，Ctrl+C 仍作「再复制 / 无选区则中断」。
Textual Screen 基类把 ctrl+c 绑为 copy_text；按键 BINDINGS 在 widget on_key
之前检查——若不在这里也接管 ctrl+c，聚焦本面板时 Ctrl+C 永远被复制吞掉、
发不到托管会话。策略：有选中文本就复制，没有就转发给托管会话。
